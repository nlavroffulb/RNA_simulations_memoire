...
std::vector<std::vector<double>> rosenbluth_grow_chain(std::vector<double>& starting_end,
	...
	std::vector<std::vector<double>>& excluded_volumes, int trials, double& config_prob, unbound_section* section) {
	...
	unbound_section* grown_section = new unbound_section(config_prob, R_weight, config_energy, 0);
...
std::vector<std::vector<double>> rosenbluth_random_walk(std::vector<double>& starting_end, int monomers, std::vector<std::vector<double>>& excluded_volumes, unbound_section* section)
{
	...
	unbound_section* grown_section = new unbound_section(config_prob, weight, energy, 1);
	section = grown_section;
...
unbound_section::unbound_section(double prob, double weight, std::vector<std::vector<double>> position_vectors, bool rw)
{
	P_R = prob;
	W_R = weight;
...
unbound_section::unbound_section(double prob, double weight, double U_R, bool rw)
{
	P_R = prob;
	W_R = weight;
	random_walk = rw;
...
unbound_section::unbound_section(std::vector<int>& limits, std::vector<double>& weight_vals, std::vector<double>& energy_vals, bool rw) {
	monomer_limits = limits;
	weights = weight_vals;
	energies = energy_vals;
	random_walk = rw;
}
unbound_section::~unbound_section() {
	std::cout << "Unbound section destructor called" << std::endl;
}

double unbound_section::get_config_prob()
{
	if (P_R == NULL) {
...
double unbound_section::get_rosen_weight()
{
	if (W_R == NULL) {
...
double unbound_section::get_energy()
{
	if (U == NULL) {
...
std::vector<std::vector<double>> unbound_section::get_positions()
{
	return positions;
}

bool unbound_section::get_growth_method()
{
	return random_walk;
}

double unbound_section::subsection_weight(std::vector<int> limit)
{
...
double unbound_section::subsection_energy(std::vector<int> limit)
{
	//check that the subsection is inside the limit. the limit argument must always be rising. we give limits of the form [a,b] where a<b always.
	if (limit[0]<monomer_limits[0] || limit[1]>monomer_limits[1]) {
		std::cout << "invalid limits in argument. return NULL." << std::endl;
...
void unbound_section::modify_weights(std::vector<int> limits, std::vector<double> new_weights)
{
}

void unbound_section::modify_energies(std::vector<int> limits, std::vector<double> new_energies)
{
}
